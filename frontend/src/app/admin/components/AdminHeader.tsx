...
import { Bell, User, Moon, Sun, X, Menu } from 'lucide-react';
...
export default function AdminHeader({ onMenuClick }: AdminHeaderProps) {
  ...
  return (
    <header className="fixed top-0 right-0 left-0 lg:left-64 h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between lg:justify-end px-4 lg:px-8 gap-2 lg:gap-4 transition-colors z-40">
      <button onClick={onMenuClick} className="lg:hidden p-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800">
      ...
      <div className="flex items-center gap-2 lg:gap-3 px-2 lg:px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-lg transition-colors">
        ...
        <span className="text-sm font-medium text-slate-900 dark:text-white hidden sm:inline">{adminEmail}</span>
      </div>
      </div>
    </header>
  );
